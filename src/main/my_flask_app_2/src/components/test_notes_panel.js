...
const ResultsScreen = () => {
    ...
    rows.forEach((row, index) => {
        if (index > 0) { // Skip the header row
            ...
            if (notesCol && notesCol.innerText.trim()) {  // If the Notes column has content
              ...
              notesCol.innerHTML = `<a href="#" onclick="handleNoteClick('${noteContent}')">${noteContent}</a>`;
            }
  ...
  const handleNoteClick = (noteContent) => {
    alert('inside note click')
    //setSelectedNote(responseData.notes[noteKey]);  // Set the selected note content
  };
